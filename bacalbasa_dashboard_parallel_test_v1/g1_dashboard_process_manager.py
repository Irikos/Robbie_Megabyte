...
class ControllerProcessManager:
    ...
    def ensure_dashboard_fullbody_sender(self) -> dict[str, Any]:
        """Keep full-body telemetry alive only when process actions are enabled."""
        with self._lock:
            current = self.fullbody_sender.status()

            # BACALBASA PARALLEL READ-ONLY GUARD
            #
            # The production Step 4.8 manager historically kept the sender alive
            # for the dashboard lifetime.  A read-only parallel dashboard must
            # never create robot-side helper processes merely because the browser
            # queried /api/controller.
            if not self.enabled:
                return current
    ...
    def camera_status(self) -> dict[str, Any]:
        with self._lock:
            ...
            # FULL_DASH_CAMERA_STATE_REACQUIRE_V21_4
            #
            # The dashboard state file contains the exact PID + Linux
            # process start ticks.  If in-memory ownership is ever lost
            # while that exact runner is still alive, reload the persisted
            # ownership record rather than misclassifying our own runner
            # as RUNNING_EXTERNAL.
            #
            # Successful managed shutdown removes this state file, so this
            # does not adopt unrelated/manual camera processes.
            if (
                self._camera_managed is None
                and self.camera_state_path.is_file()
            ):
                self._load_camera_state()
                self._refresh_camera()
    ...
    def status(self) -> dict[str, Any]:
        with self._lock:
            ...
            now = time.time()
            inspire = self._inspire_status()
            # FULL_DASH_STATUS_PASSIVE_V21_3
            # API/status reads must never create processes.
            fullbody = self.fullbody_sender.status()
