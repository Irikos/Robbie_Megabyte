        ...
        with self._lock:
            ...
            return {
                ...
                "robot": {
                    ...
                    # FULL_DASH_BMS_SOC_V20_5
                    "battery_soc": robot.get("battery_soc"),
                },
                "arms": {
...
# FULL_DASH_CAMERA_PROXY_V2
#
# The browser talks only to the Full Dash on port 8082.
# Teleimager HTTPS signaling is contacted only through localhost
# by this bridge, so the browser never sees its self-signed cert.

FULL_DASH_CAMERA_PORTS = {
    "rgb": 60001,
    "depth": 60005,
    "overlay": 60006,
    "disparity": 60008,
...
def full_dash_read_body(handler):
    length = int(
        handler.headers.get(
            "Content-Length",
            "0",
...
def full_dash_write_views(raw=None):
    ...
    selected = [
        v
        for v in FULL_DASH_CAMERA_VIEWS
        if v in wanted
    ]

    if not selected:
        raise ValueError(
            "no camera views selected"
        )

    path = full_dash_views_path()
...
def full_dash_prepare_camera(manager, raw_views=None):

    selected = full_dash_write_views(
        raw_views
    )

    status = manager.camera_status()
    ...
    if state not in {
        ...
        # Camera lifecycle only.
        # No controller action method is called here.
        camera_manager = ControllerProcessManager(
            enabled=True
        )

        status = camera_manager.camera_status()
...
def full_dash_offer_view(raw_path):
...
# FULL_DASH_SDP_DEBUG_V1
def full_dash_log_sdp(kind, view, sdp):

    path = Path(
        os.environ.get(
            "G1_DASHBOARD_CAMERA_SDP_LOG",
            "/tmp/full_dash_camera_webrtc_sdp.log",
        )
    )
...
# FULL_DASH_FORWARD_BROWSER_UA_V1
def full_dash_proxy_offer(view, payload, browser_user_agent=""):

    import ssl
    import urllib.error
    import urllib.request

    port = FULL_DASH_CAMERA_PORTS.get(
        view
    )

    if port is None:
        raise ValueError(
            f"invalid WebRTC view: {view!r}"
        )
    ...
    if browser_user_agent:
        teleimager_headers["User-Agent"] = (
            browser_user_agent
        )

    request = urllib.request.Request(
        f"https://127.0.0.1:{port}/offer",
        data=body,
        headers=teleimager_headers,
        method="POST",
    )
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def _trusted_request_origin_ok(self) -> bool:
        import ipaddress
        ...
        if origin:
            ...
            # FULL_DASH_NATIVE_TRUST_V20_4
            #
            # Browser clients normally provide Sec-Fetch-Site.
            # Native companion applications generally do not.
            #
            # An absent value is allowed here because the request
            # is still restricted below to loopback/private-network
            # source addresses. Protected action endpoints continue
            # to require the issued trusted session.
            site = self.headers.get(
                "Sec-Fetch-Site",
                "",
            ).strip().lower()
    ...
    def _trusted_session_token(self) -> str:
    ...
    def _trusted_session_authorized(self) -> bool:
        import secrets

        if not self._trusted_request_origin_ok():
            return False

        token = self._trusted_session_token()
        ...
        sessions = (
            self.server.trusted_sessions
        )  # type: ignore[attr-defined]

        lock = (
            self.server.trusted_sessions_lock
        )  # type: ignore[attr-defined]
    ...
    def _process_action_authorized(self) -> bool:
        ...
        # Preferred Full Dash authorization.
        if self._trusted_session_authorized():
            return True

        # Temporary V14 rollback/fallback only.
        expected: str = (
    ...
    def _require_process_action_auth(self) -> bool:
        ...
        if not self._process_action_authorized():
            self._send_json(
                ...
                {
                    "error":
                    "trusted dashboard session is missing or invalid"
                },
            )
            return False
        ...
        if path == "/api/session/stream":
            ...
            if (
                ...
                self._send_json(
                    ...
                    {
                        "error":
                        "invalid browser stream session_id"
                    },
                )
                return
            ...
            stream_lock = (
                self.server.dashboard_stream_lock
            )  # type: ignore[attr-defined]

            streams = (
                self.server.dashboard_streams
            )  # type: ignore[attr-defined]
            ...
            with stream_lock:
                ...
                self.server.dashboard_stream_had_client = True  # type: ignore[attr-defined]

                self.server.dashboard_stream_zero_since = None  # type: ignore[attr-defined]


            manager: ControllerProcessManager = (
                self.server.process_manager
            )  # type: ignore[attr-defined]


            try:

                manager.ensure_dashboard_fullbody_sender()

            except Exception as exc:

                print(
                    "WARNING: browser stream fullbody start failed: "
                    f"{exc}"
                )
            ...
            try:
                ...
                self.server.start_unity_compat()  # type: ignore[attr-defined]

            except Exception as exc:

                print(
                    "WARNING: browser stream Unity compatibility "
                    f"start failed: {exc}"
                )
            ...
            try:
                ...
                with stream_lock:
                    ...
                    if not streams:
                        ...
                        self.server.dashboard_stream_zero_since = now  # type: ignore[attr-defined]

                print(
                    "Browser stream closed: "
                    f"{session_id}"
                )
    ...
    def do_POST(self) -> None:
        ...
        #
        # Camera viewing only.
        # No robot/controller/service action authorization is enabled.

        if path == "/api/camera/passive/prepare":

            try:
                payload = full_dash_read_body(
                    self
                )

                views, status = (
                    full_dash_prepare_camera(
                        manager,
                        payload.get("views"),
                    )
                )
        ...
        # FULL_DASH_PASSIVE_YOLO_V2
        if path == "/api/camera/passive/yolo":
            try:
                payload = full_dash_read_body(self)
                ...
                # Camera-only request.
                # No controller / robot action path is enabled.
                manager._write_camera_yolo(enabled)

                self._send_json(
                    HTTPStatus.OK,
                    manager.camera_status(),
                )
        ...
        if path == "/api/camera/passive/offer":
            ...
            try:
                ...
                view = full_dash_offer_view(
                    self.path
                )

                answer = full_dash_proxy_offer(
                    view,
                    payload,
                    self.headers.get(
                        "User-Agent",
        ...
        # This endpoint never sends DDS robot commands.
        #
        ...
        if path == "/api/session/lease":
            ...
            if not self._trusted_request_origin_ok():
                self._send_json(
                    ...
                    {
                        "error":
                        "dashboard lease requires a same-origin private-network browser"
                    },
                )
                return
            ...
            try:
                ...
                self._send_json(
                    ...
                    {
                        "error":
                        f"invalid dashboard lease request: {exc}"
                    },
                )
                return
            ...
            if not isinstance(payload, dict):
                self._send_json(
                    ...
                    {
                        "error":
                        "dashboard lease payload must be an object"
                    },
                )
                return
            ...
            if operation not in {
                ...
                self._send_json(
                    ...
                    {
                        "error":
                        "operation must be open, ping, or close"
                    },
                )
                return
            ...
            leases = (
                self.server.dashboard_leases
            )  # type: ignore[attr-defined]

            lease_lock = (
                self.server.dashboard_lease_lock
            )  # type: ignore[attr-defined]
            ...
            with lease_lock:
                ...
                if operation in {
                    ...
                    leases[session_id] = now
                    self.server.dashboard_lease_had_client = True  # type: ignore[attr-defined]

                elif operation == "close":
                    # FULL_DASH_REFRESH_SAFE_LEASE_V20_9
                    # Do not remove immediately: reload/pagehide can
                    # otherwise race the replacement page and stop
                    # a live controller. With no further heartbeat,
                    # the watchdog expires this lease normally.
                    leases[session_id] = now
            ...
            if operation in {
                ...
                try:
                    fullbody = (
                        manager.fullbody_sender.status()
                    )
                    fullbody_state = (
                        fullbody.get("state")
                    )
                ...
                try:
                    compat_state = (
                        self.server.unity_compat_state
                    )  # type: ignore[attr-defined]
                except Exception as exc:
                    compat_state = (
                        f"error:{exc}"
        ...
        # Full Dash automatic trusted-session bootstrap.
        #
        # No robot/controller action occurs here.
        # ------------------------------------------------
        ...
        if path == "/api/session/bootstrap":
            ...
            if not self._trusted_request_origin_ok():
                self._send_json(
                    ...
                    {
                        "error":
                        "trusted-session bootstrap requires a same-origin private-network browser"
                    },
                )
                return
            ...
            payload = {
                ...
                "auth_mode":
                    "same-origin-private-network-session",
            }
...
def main() -> int:
    ...
    # FULL_DASH_LEASE_OWNED_FULLBODY_V20_7
    fullbody_status = process_manager.fullbody_sender.status()
    ...
    # FULL_DASH_TRUSTED_SESSION_V14
    server.trusted_sessions = {}  # type: ignore[attr-defined]
    server.trusted_sessions_lock = threading.Lock()  # type: ignore[attr-defined]


    # FULL_DASH_BROWSER_LEASE_SERVER_V20_7
    server.dashboard_leases = {}  # type: ignore[attr-defined]
    server.dashboard_lease_lock = threading.Lock()  # type: ignore[attr-defined]
    server.dashboard_lease_had_client = False  # type: ignore[attr-defined]
    server.dashboard_lease_cleanup_running = False  # type: ignore[attr-defined]
    # FULL_DASH_BROWSER_STREAM_V21_2B
    #
    # An actually open HTTP/EventSource connection represents an
    # open Full Dash browser page. No JavaScript heartbeat timer
    # determines ownership.
    server.dashboard_streams = set()  # type: ignore[attr-defined]
    server.dashboard_stream_lock = threading.Lock()  # type: ignore[attr-defined]
    server.dashboard_stream_had_client = False  # type: ignore[attr-defined]
    server.dashboard_stream_zero_since = None  # type: ignore[attr-defined]
    server.dashboard_stream_cleanup_running = False  # type: ignore[attr-defined]

    server.unity_compat_server = None  # type: ignore[attr-defined]
    server.unity_compat_lock = threading.Lock()  # type: ignore[attr-defined]
    server.unity_compat_state = "STOPPED"  # type: ignore[attr-defined]


    class _UnityCompatHTTPServer(
        ThreadingHTTPServer
    ):
        allow_reuse_address = True


    class _UnityCompatHandler(
        BaseHTTPRequestHandler
    ):
        server_version = "G1UnityCompat/1.0"
        protocol_version = "HTTP/1.1"
        ...
        def do_GET(
            ...
            # FULL_DASH_UNITY_DIRECT_SNAPSHOT_V20_8
            #
            # Read-only compatibility surface for the Quest app.
            # No internal HTTP proxy and no action endpoint.
            #
            ...
            try:
                payload = unity_telemetry_snapshot(
                    state,
                    system_state,
                )
                ...
                body = json.dumps(
                    {
                        "error":
                        "Unity telemetry snapshot failed: "
                        + str(exc)
                    },
                    separators=(
                        ",",
    ...
    def _start_unity_compat() -> str:

        with server.unity_compat_lock:  # type: ignore[attr-defined]

            existing = (
                server.unity_compat_server
            )  # type: ignore[attr-defined]
            ...
            try:
                ...
                server.unity_compat_state = (  # type: ignore[attr-defined]
                    f"PORT_BUSY:{exc}"
                )
                return (
                    server.unity_compat_state
                )  # type: ignore[attr-defined]

            server.unity_compat_server = compat  # type: ignore[attr-defined]
            server.unity_compat_state = "RUNNING"  # type: ignore[attr-defined]

            thread = threading.Thread(
                target=
                    compat.serve_forever,
                kwargs={
                    "poll_interval":
                    0.25,
                },
            ...
            print(
                ...
                "http://0.0.0.0:8080"
                "/api/unity/telemetry "
                "(read-only, browser-lease owned)"
            )

            return "RUNNING"


    def _stop_unity_compat() -> str:
    ...
    def _dashboard_lease_watchdog() -> None:
        ...
        # FULL_DASH_BROWSER_STREAM_WATCHDOG_V21_2B
        #
        # Browser lifetime comes from the persistent HTTP
        # connection. Background-tab timer throttling cannot stop
        # controller/camera/fullbody resources.
        #
        CLOSE_GRACE_S = 4.0
        ...
        while True:
            ...
            with server.dashboard_stream_lock:  # type: ignore[attr-defined]
                ...
                streams = (
                    server.dashboard_streams
                )  # type: ignore[attr-defined]

                zero_since = (
                    server.dashboard_stream_zero_since
                )  # type: ignore[attr-defined]

                should_cleanup = bool(
                    server.dashboard_stream_had_client  # type: ignore[attr-defined]
                    and not streams
                    and zero_since is not None
                    and (
                        now
                        - float(
                            zero_since
                    ...
                    and not server.dashboard_stream_cleanup_running  # type: ignore[attr-defined]
                )

                if should_cleanup:

                    server.dashboard_stream_cleanup_running = True  # type: ignore[attr-defined]
            ...
            # Refresh/navigation race window.
            time.sleep(
                1.0
            )
            ...
            with server.dashboard_stream_lock:  # type: ignore[attr-defined]
                ...
                if server.dashboard_streams:  # type: ignore[attr-defined]
            ...
            try:
                ...
                print(
                    "WARNING: Unity compatibility cleanup failed: "
                    f"{exc}"
                )
            ...
            try:
                ...
                summary = (
                    process_manager.shutdown_dashboard_managed(
                        timeout_s=15.0
                    )
                )

                print(
                    "Browser stream cleanup: "
                    f"{summary}"
                )
                ...
                print(
                    "WARNING: browser stream cleanup failed: "
                    f"{exc}"
                )
            ...
            with server.dashboard_stream_lock:  # type: ignore[attr-defined]
                ...
                server.dashboard_stream_had_client = active_again  # type: ignore[attr-defined]
            ...
            if active_again:
                ...
                try:
                    ...
                    process_manager.ensure_dashboard_fullbody_sender()

                except Exception as exc:

                    print(
                        "WARNING: post-cleanup fullbody restart failed: "
                        f"{exc}"
                    )
                ...
                try:
                    ...
                    print(
                        "WARNING: post-cleanup Unity restart failed: "
                        f"{exc}"
                    )
    ...
    if args.enable_process_actions:
        ...
        print("Auth            : automatic same-origin trusted browser session")
