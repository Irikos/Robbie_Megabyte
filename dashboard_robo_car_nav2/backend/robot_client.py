...
via mesaje ROS 2 unitree_api pe rețeaua internă 192.168.123.x.
Importul acestui modul nu inițializează SDK/DDS și nu trimite comenzi.
"""
...
class SlamClient:
    ...
    def __init__(self):
        ...
        self._ros_available = False  # transportul este legat la startup
...
# -----------------------------------------------------------------------
# Sport Client (mișcare robot) via ROS 2
# -----------------------------------------------------------------------
class SportClient:
    """
    Controlează mișcarea robotului G1 prin transportul ROS 2 al backendului.
    """
    ...
    def __init__(self):
        ...
        # Importul modulului nu deschide DDS. Backendul leagă transportul
        # ROS după inițializarea unică a contextului rclpy.
        self._sdk_available = False

    def initialize_ros(self, node) -> None:
        """Leagă interfața istorică de publisherii ROS ai motorului Nav2.

        Numele atributelor SDK rămân pentru compatibilitatea UI/testelor;
        niciun modul unitree_sdk2py nu este importat sau inițializat.
        """
        from ros_sport_client import ARM_ACTIONS, RosSportClient
        self._sdk_client = RosSportClient(node)
        self._arm_action_client = RosSportClient(node, service="arm", timeout=10.0)
        self._arm_action_map = dict(ARM_ACTIONS)
        self._client_kind = "ros2"
        self._sdk_available = True
        print("[SportClient] Transport ROS 2 activ (sport + arm), fără SDK DDS.")
