...
def test_geometric_alignment_recovers_rectangular_room():
    # A single rectangular room (4 walls) where SIFT would otherwise struggle with symmetry
    walls = []
    for x in np.linspace(0, 6, 80):
        walls.append((x, 0))
        walls.append((x, 4.5))
    for y in np.linspace(0, 4.5, 60):
        walls.append((0, y))
        walls.append((6, y))

    target = np.asarray(walls)
    expected_x, expected_y, expected_yaw = 1.8, -2.2, math.radians(34.0)
    source = _transform(target, -expected_x, -expected_y, -expected_yaw)

    result = align_point_maps(target, source, resolution=0.06)

    assert result["accepted"] is True
    assert result["overlap"] > 0.50
    assert result["rmse_m"] < 0.15
