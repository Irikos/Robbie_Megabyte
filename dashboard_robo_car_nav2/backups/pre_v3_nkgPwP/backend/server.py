...
# Navigația A* nativă a fost ELIMINATĂ complet din robot+car.
# Toată navigarea autonomă folosește acum EXCLUSIV motorul Nav2 (identic cu
# dashboard_g1_nav2_v3): Nav2 planifică ruta, iar comanda ajunge la robot numai
# prin adaptorul ROS 2 unitree_api (Sport API 7105). API-urile native de
# navigație 1102/1201/1202 nu mai sunt folosite niciodată.
from nav2_engine import engine as nav2_engine
from nav2_observer import Nav2ObserverPublisher

# Rasterizorul PCD->ocupare 2D (fostul PCDGridPlanner) este păstrat FĂRĂ
# planificarea A*, fiindcă localizarea LiDAR locală și previzualizarea
# costmap-ului îl folosesc pentru geometria hărții.
from pcd_grid_map import PCDGridPlanner

# Executoarele A* (AutonomousNavigator/NativeWaypointNavigator) au fost
# ELIMINATE; numele rămân definite doar ca placeholder pentru adnotări reziduale.
AutonomousNavigator = NativeWaypointNavigator = object


def plan_pcd_route(*_args, **_kwargs):
    raise RuntimeError("Planificatorul A* a fost eliminat; navigarea folosește Nav2")


def _map_nav2_state(state: str) -> str:
    """Traduce starea internă Nav2 în vocabularul de status al dashboardului."""
    if state in ("starting", "navigating", "orienting"):
        return "navigating"
    if state == "completed":
        return "arrived"
    if state == "failed":
...
class Nav2Navigator:
    """Adaptor de compatibilitate: expune interfața de navigator folosită de
    dashboard (``start``/``stop``/``status``/``task``/``confirm_replan``), dar
    conduce motorul Nav2 (``nav2_engine``) în locul planificatorului A*.

    Un singur task de monitorizare urmărește starea Nav2 și emite exact
    aceleași evenimente ca vechiul executor (``navigating``/``arrived``/
    ``failed``/``cancelled``), astfel încât gesturile după sosire și coada FIFO
    continuă să funcționeze neschimbate.
    """
    ...
    async def start(self, *args, **kwargs) -> dict:
        # Semnătura istorică era start(map_path, x, y, yaw, speed=, timeout=,...).
        positional = list(args)
        if len(positional) >= 4:
            _map_path, gx, gy, gyaw = positional[0], positional[1], positional[2], positional[3]
        else:
            gx = kwargs.get("x", 0.0)
            gy = kwargs.get("y", 0.0)
            gyaw = kwargs.get("yaw", 0.0)
        speed = float(kwargs.get("speed", 0.22) or 0.22)
        result = await nav2_engine.goal(float(gx), float(gy), float(gyaw), speed)
        if result.get("success"):
            self._ensure_monitor()
        return result

    async def stop(self, reason: str = "") -> dict:
        result = await nav2_engine.stop()
        if self._task and not self._task.done():
            self._task.cancel()
        self._task = None
        self._last_state = "idle"
        return {"success": bool(result.get("success", True)),
                "output": reason or "STOP Nav2", **result}

    async def confirm_replan(self, confirmation_id: str = "") -> dict:
        return {"success": True,
                "message": "Nav2 replanifică automat; nu este nevoie de confirmare manuală"}

    async def reject_replan(self, confirmation_id: str = "") -> dict:
        return {"success": True,
                "message": "Nav2 gestionează intern replanificarea și recovery-ul"}
    ...
    async def _monitor(self) -> None:
        active = {"starting", "navigating", "orienting"}
        try:
            while True:
                await asyncio.sleep(0.2)
                snapshot = nav2_engine.status()
                state = snapshot.get("state", "idle")
                mapped = _map_nav2_state(state)
                if mapped != self._last_state:
                    self._last_state = mapped
                    await _navigation_event({
                        "type": "nav_status", "state": mapped, "driver": "nav2",
                        "goal": snapshot.get("goal"),
                        "message": snapshot.get("message", ""),
                        "remaining": snapshot.get("remaining"),
...
# Motor unic de navigație Nav2, expus prin interfața de navigator.
_nav2_navigator = Nav2Navigator()
...
if ROS2_AVAILABLE:
    class DualLidarSLAMSubscriber(Node):
        def __init__(self):
            ...
            # Observerul vechi este opțional; motorul Nav2 propriu-zis este
            # inițializat separat la startup, indiferent de acest flag.
            if os.environ.get("G1_ENABLE_NAV2_OBSERVER") == "1":
                ...
                self.get_logger().info(
                    "Observerul vechi dezactivat; navigarea folosește motorul Nav2"
                )
        ...
        def odom_pelvis_callback(self, msg):
            ...
            # Alimentează Nav2 cu odometria continuă a pelvisului. update_local_odometry
            # publică /odom și TF odom→base_link + ancora map→odom pentru planner.
            try:
                nav2_engine.update_local_odometry(msg)
            except Exception:
                pass
        ...
        def _process_odom_coords(self, x: float, y: float, yaw: float, source: str = "odom"):
            ...
            # Alimentează Nav2 cu poza de localizare (map→base_link). Ancora
            # map→odom se calculează din această poză și odometria pelvisului.
            try:
                nav2_engine.update_map_pose(map_state["pose"])
            except Exception:
                pass
            if self.nav2_observer:
...
camera_receiver_ready = threading.Event()
...
async def get_agents_status():
    ...
    robot_pose = dict(map_state.get("pose") or {})
    robot_updated_at = float(map_state.get("pose_updated_at") or 0.0)
    robot_pose_valid = _pose_xy(robot_pose) is not None
    robot_age = max(0.0, now - robot_updated_at) if robot_updated_at else None
    robot_online = bool(
        robot_pose_valid
    ...
    vehicle_pose_valid = _pose_xy(vehicle_pose) is not None
...
def _agents_static_cloud_snapshot(maximum: int = 24000) -> dict:
    ...
    if current_map:
        raw_points = slam_client.read_pcd_points(current_map)
        valid = []
...
async def robot_status():
    """Diagnostic transport ROS 2; cheia sdk_available rămâne pentru UI-ul vechi."""
...
def _get_autonomous_navigator():
    """Întoarce motorul unic Nav2 (fostul executor A* local a fost eliminat)."""
    global autonomous_navigator
    autonomous_navigator = _nav2_navigator
    return _nav2_navigator


# --- Executoarele de navigație nativă (A* + API 1102/1201/1202) au fost
#     ELIMINATE. Navigarea autonomă folosește exclusiv nav2_engine. ---
...
def _get_native_waypoint_navigator():
    """Compatibilitate: navigatorul nativ 1102 a fost eliminat. Toate apelurile
    primesc același motor Nav2, singura cale de navigare rămasă."""
    global native_waypoint_navigator
    native_waypoint_navigator = _nav2_navigator
    return _nav2_navigator
...
def _planar_map_points_for_nav2(map_path: str):
    """Extrage un contur 2D din PCD pentru rasterizarea `/map`.

    `occupancy_from_points` nu filtrează pe înălțime, deci trimitem numai
    punctele dintr-o bandă de înălțime în jurul corpului (pereți/obstacole),
    proiectate în plan. Banda este configurabilă prin variabile de mediu.
    """
    from pcd_io import read_pcd
    points = read_pcd(Path(map_path))
    if not points:
        raise ValueError("PCD-ul hărții nu conține puncte")
    floor = min(p[2] for p in points)
    z_lo = floor + float(os.environ.get("ROBOTCAR_NAV2_MAP_MIN_Z", "0.15"))
    z_hi = floor + float(os.environ.get("ROBOTCAR_NAV2_MAP_MAX_Z", "1.80"))
    sliced = [(p[0], p[1], 0.0) for p in points if z_lo <= p[2] <= z_hi]
    if len(sliced) < 20:
        # Bandă prea îngustă: proiectează întreaga hartă în plan.
        sliced = [(p[0], p[1], 0.0) for p in points]
    return sliced


_nav2_map_published_for = {"path": None}


def _ensure_nav2_map(map_path: str, origin_xy) -> dict:
    """Publică `/map` pentru Nav2 o singură dată per hartă încărcată."""
    if _nav2_map_published_for.get("path") == map_path and nav2_engine.map_ready:
        return {"cached": True}
    points = _planar_map_points_for_nav2(map_path)
    grid = nav2_engine.set_map(points, origin_xy)
    _nav2_map_published_for["path"] = map_path
    return grid


def _invalidate_nav2_map() -> None:
    _nav2_map_published_for["path"] = None


async def _preview_navigation(body: dict, allow_active: bool = False):
    """Calculează ruta Nav2 (ComputePathToPose) fără mișcare și cere confirmarea.

    Nu mai există planner A*; ruta o produce exclusiv Nav2, la fel ca în v3.
    """
    ...
    try:
        await asyncio.to_thread(_ensure_nav2_map, map_path, (pose["x"], pose["y"]))
    except Exception as exc:
        return {"success": False, "error": f"Harta Nav2 nu poate fi publicată: {exc}"}
    preview = await nav2_engine.preview(x, y, yaw, timeout or 12.0)
    if not preview.get("success"):
        return {
            "success": False,
            "error": preview.get("error", "Nav2 nu a găsit o rută către țintă"),
            "diagnostics": {"driver": "nav2", "start_pose": pose, "requested_goal": requested_goal},
        }
    path = preview["path"]
    preview_id = secrets.token_urlsafe(18)
    path_length = float(preview.get("distance") or sum(
    ...
    pending_nav_previews = {
        preview_id: {
            ...
            "requested_goal": requested_goal, "goal_adjustment_m": 0.0,
            "speed": speed, "timeout": timeout, "path": path, "planner": None,
            "clearance_mode": "nav2", "costmap_settings": {},
            "gesture": gesture, "path_pattern": {"type": "nav2_gridbased"},
            "driver": "nav2", "executor": "nav2",
            "requested_executor": "nav2", "executor_override": None, "nav2_plan": None,
        }
    }
    return {
        ...
        "gesture": gesture, "path_pattern": {"type": "nav2_gridbased"},
        "driver": "nav2", "executor": "nav2",
        "requested_executor": "nav2", "executor_override": None,
...
async def start_navigation(body: dict = Body({})):
    """Execută ruta Nav2 previzualizată și confirmată de operator.

    Motorul de mișcare este identic cu v3: NavigateToPose + replanificare, cu
    comanda finală trimisă la robot numai prin adaptorul 7105.
    """
    ...
    if _any_navigation_active():
        return {"success": False, "error": "Există deja o rută Nav2 activă; apasă STOP înainte de o rută nouă."}
    pose = {**dict(map_state["pose"]), "source": map_state.get("pose_source")}
    ...
    active_navigation_driver = "nav2"
    active_navigation_executor = "nav2"
    flight_log = _start_navigation_flight_log(preview, pose)
    result = await nav2_engine.goal(
        float(goal["x"]), float(goal["y"]), float(goal["yaw"]), float(preview["speed"])
    )
    if result.get("success"):
        # Pornește monitorizarea de stare (gesturi la sosire + coadă FIFO).
        _get_autonomous_navigator()._ensure_monitor()
        result.update({
            "api_id": None,
            "executor": "nav2",
            "driver": "nav2_gridbased",
            "planner": "Nav2 GridBased",
            "map_path": map_path,
...
async def nav2_status():
    ...
    if not snapshot.get("available"):
        return {"success": False, "available": False, "error": "Motorul Nav2 nu este pornit încă"}
...
async def navigation_preflight(
        ...
        driver: str = "nav2",
        executor: str = "nav2"):
    """Preflight Nav2: harta, localizarea, senzorii de obstacol și starea
    motorului Nav2 (adaptor, TF map→odom, hartă publicată, scan proaspăt) plus
    FSM-ul locomotor. A* și API-urile native 1102 au fost eliminate, deci nu mai
    există verificări de navigație nativă."""
    ...
    while True:
        ...
        try:
            local_fsm_id = await asyncio.to_thread(sport_client.get_current_fsm_id)
        except Exception:
            local_fsm_id = None
        nav2_details = nav2_engine.status()
        scan_age = nav2_details.get("scan_age")
        checks = {
            ...
            "nav2_map_published": bool(nav2_details.get("map_ready")),
            "nav2_scan_fresh": bool(scan_age is not None and scan_age <= 0.6),
        ...
        if success or non_sensor_failure or elapsed >= wait_s:
            return {
                ...
                "robot": {
                    "fsm_id": local_fsm_id,
                    "locomotion_ready": local_fsm_id in {500, 501, 502, 801, 802},
                },
                "nav2": nav2_details,
...
async def startup_health():
    ready = (ROS2_AVAILABLE and rclpy.ok() and node_instance is not None
             and nav2_engine._node is not None and camera_receiver_ready.is_set())
...
async def startup_event():
    ...
    if not ROS2_AVAILABLE:
        raise RuntimeError("ROS 2/unitree_api indisponibil: pornește prin start_dashboard.sh")
    # Un singur context DDS, inițializat înaintea threadurilor. O eroare ROS
    # trebuie să oprească startupul, nu să lase un dashboard aparent funcțional.
    if not rclpy.ok():
        # Uvicorn administrează SIGINT/SIGTERM. Păstrăm DDS activ până după
        # bariera de viteză zero din shutdown_event.
        rclpy.init(signal_handler_options=SignalHandlerOptions.NO)
    nav2_engine.ensure_started(loop)
    sport_client.initialize_ros(nav2_engine._node)
    node_instance = DualLidarSLAMSubscriber()
    threading.Thread(target=ros2_thread_entry, daemon=True).start()
    print("[nav2] Motorul Nav2 a pornit (ROS 2, fără SDK DDS).")
...
async def shutdown_event():
    ...
    if navigation_post_action_task and not navigation_post_action_task.done():
        ...
        try:
            ...
            pass
        navigation_post_action_task = None
    # Oprește ruta Nav2 și trimite viteză zero prin adaptorul 7105.
    try:
        await nav2_engine.stop()
    except Exception:
        pass
    # Barieră locomotorie finală prin același transport ROS 2.
    await asyncio.to_thread(sport_client.stop)
...
def ros2_thread_entry():
    ...
    try:
        ...
        pass  # închiderea normală a contextului, nu o eroare de startup
